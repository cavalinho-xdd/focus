...
// Well-known distraction apps — shown first in the process picker.
// match[] entries are lowercased substrings of the process name.
const KNOWN_DISTRACTIONS = [
  { display: 'Discord',           match: ['discord'] },
  { display: 'Slack',             match: ['slack'] },
  { display: 'Telegram',          match: ['telegram'] },
  { display: 'WhatsApp',          match: ['whatsapp'] },
  { display: 'Signal',            match: ['signal'] },
  { display: 'Microsoft Teams',   match: ['teams'] },
  { display: 'Zoom',              match: ['zoom'] },
  { display: 'Skype',             match: ['skype'] },
  { display: 'Steam',             match: ['steam'] },
  { display: 'Epic Games',        match: ['epicgameslauncher', 'epicgames'] },
  { display: 'Battle.net',        match: ['battle.net', 'battlenet'] },
  { display: 'Origin / EA',       match: ['origin', 'eadesktop', 'ea desktop'] },
  { display: 'GOG Galaxy',        match: ['galaxyclient'] },
  { display: 'Ubisoft Connect',   match: ['ubisoftconnect', 'uplay'] },
  { display: 'Chrome',            match: ['chrome'] },
  { display: 'Firefox',           match: ['firefox'] },
  { display: 'Edge',              match: ['msedge'] },
  { display: 'Safari',            match: ['safari'] },
  { display: 'Brave',             match: ['brave'] },
  { display: 'Opera',             match: ['opera'] },
  { display: 'Arc',               match: ['arc'] },
  { display: 'Spotify',           match: ['spotify'] },
  { display: 'Twitch',            match: ['twitch'] },
  { display: 'VLC',               match: ['vlc'] },
  { display: 'YouTube Music',     match: ['youtubemusic'] },
  { display: 'X / Twitter',       match: ['twitter'] },
  { display: 'Reddit',            match: ['reddit'] },
  { display: 'League of Legends', match: ['leagueclient'] },
  { display: 'Valorant',          match: ['valorant'] },
  { display: 'Fortnite',          match: ['fortniteclient', 'fortnite'] },
  { display: 'Minecraft',         match: ['minecraft'] },
];

function matchDistraction(procName) {
  const lower = procName.toLowerCase();
  return KNOWN_DISTRACTIONS.find(d => d.match.some(m => lower.includes(m)));
}
import { useTranslation } from 'react-i18next';
import { motion, AnimatePresence } from 'framer-motion';
import ToggleSwitch from './ToggleSwitch';

function Settings({ settings, onSave, onForceTour }) {
  ...
  return (
    <div className="max-w-2xl w-full flex flex-col">
      ...
      >
        <label htmlFor="apiKey" className="flex items-center gap-2 mb-3 text-sm text-gray-500 font-medium tracking-wide">
      ...
      >
        <label htmlFor="blacklist" className="flex items-center gap-2 mb-3 text-sm text-gray-500 font-medium tracking-wide">
      ...
      {/* Blocked Websites (HIDDEN FOR PRODUCTION) */}
      {false && (
        <motion.div 
          className="mb-8 flex flex-col"
          initial={{ opacity: 0, y: 20 }}
        ...
        >
          <label htmlFor="blockedWebsites" className="flex items-center gap-2 mb-3 text-sm text-gray-500 font-medium tracking-wide">
      ...
      >
        <button 
          type="button"
          onClick={handleSave}
          className={`w-full text-white font-bold py-4 px-8 rounded-xl btn-press flex items-center justify-center gap-2 ${
            isSaved 
        ...
        <button 
          type="button"
          onClick={() => onForceTour && onForceTour()}
          className="w-full mt-4 bg-white/5 hover:bg-white/10 border border-white/10 text-gray-300 font-semibold py-3 px-8 rounded-xl btn-press flex justify-center items-center gap-2"
        >
          <Activity size={18} /> {t('settings.replayTour', 'Replay Walkthrough')}
        </button>
        ...
        <button 
          ...
          className="w-full mt-4 bg-red-500/10 hover:bg-red-500/20 border border-red-500/20 text-red-400 font-semibold py-3 px-8 rounded-xl btn-press"
        >
            ...
            >
              ...
              >
                <div className="flex justify-between items-center mb-6 shrink-0">
                  ...
                  <button type="button" onClick={() => setShowScannerModal(false)} aria-label="Close scanner modal" className="text-gray-500 hover:text-white transition-colors">
                    <X size={20} />
                  </button>
                </div>
                ...
                {(() => {
                  ...
                  const filtered = scannedProcesses.filter(p => p.toLowerCase().includes(lowerQ));
                  const distractions = filtered.filter(p => matchDistraction(p));
                  const others = filtered.filter(p => !matchDistraction(p));

                  const renderRow = (proc) => {
                    const known = matchDistraction(proc);
                    return (
                      <label key={proc} className="flex items-center gap-3 py-2.5 px-3 hover:bg-white/5 rounded-lg cursor-pointer transition-colors group">
                        <input
                          type="checkbox"
                        ...
                        <div className="flex flex-col min-w-0">
                          <span className="text-sm font-medium text-gray-200 leading-tight">
                            {known ? known.display : proc}
                          </span>
                          {known && (
                            <span className="text-[10px] font-mono text-gray-600 leading-tight">{proc}</span>
                          )}
                        </div>
                      </label>
                    );
                  ...
                  return (
                    <div className="overflow-y-auto flex-1 min-h-0 flex flex-col mb-6 custom-scrollbar">
                      {distractions.length > 0 && (
                        <div className="mb-1">
                          <p className="text-[10px] text-gray-600 font-medium px-3 pt-1 pb-2 sticky top-0 bg-[#0B0A15]/95">
                            Distraction apps
                          </p>
                          {distractions.map(renderRow)}
                        </div>
                      )}
                      {others.length > 0 && (
                        <div>
                          {distractions.length > 0 && <div className="border-t border-white/5 my-2 mx-3" />}
                          <p className="text-[10px] text-gray-600 font-medium px-3 pt-1 pb-2 sticky top-0 bg-[#0B0A15]/95">
                            Other
                          </p>
